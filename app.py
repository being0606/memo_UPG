...
#     # "이전 평가로" 버튼
#     if col1.button("이전 평가로", disabled=(idx == 0)):
#         if idx > 0:
...
#                     if st.button(f"🎈 {task} - 풍선 100개 쌓기", key=f"collect_{task_key}"):
#                         st.session_state.collectable_xp += 100
#                         st.session_state.balloons_collected[task_key] = True
#                         st.success(f"풍선 100개를 모았습니다!")
#                 elif checkbox and st.session_state.balloons_collected.get(task_key, False):
#                     st.write(f"🎈 {task} - 풍선 100개 모음 완료")
...
#                         st.session_state.balloons_collected[task_key] = True
#                         st.success(f"풍선 {balloons}개를 모았습니다!")
#                 elif checkbox and st.session_state.balloons_collected.get(task_key, False):
#                     st.write(f"🎈 {task} - 풍선 {balloons_by_category.get(category, 50)}개 모음 완료")
...
def todo_step():
    ...
    st.write(f"현재까지 모은 풍선: **{st.session_state.total_balloons}개**")
    st.progress(min(st.session_state.total_balloons / 500, 1.0))  # 500개를 기준으로 게이지 바 표시
